...
REbonf <- length(meta.res$PvalueARE[which(p.adjust(meta.res$PvalueARE,method="bonferroni")<0.05)])
REbonf.es <- range(abs(meta.res$Effect[which(p.adjust(meta.res$PvalueARE,method="bonferroni")<0.05)]))
REfdr <- length(meta.res$PvalueARE[which(p.adjust(meta.res$PvalueARE,method="fdr")<0.05)])
REfdr.es <- range(abs(meta.res$EffectARE[which(p.adjust(meta.res$PvalueARE,method="fdr")<0.05)]))
REp5 <-  length(meta.res$PvalueARE[which(meta.res$PvalueARE<1e-5)])
REp5.es <-  range(abs(meta.res$EffectARE[which(meta.res$PvalueARE<1e-5)]))
...
REbonf,REbonf.es,REfdr,REfdr.es,REp5,REp5.es,
...
"RE.N.Bonf","RE.Range.ES.Bonf","RE.N.FDR","RE.Range.ES.FDR","RE.N.P<1e-5","RE.Range.ES.P5",
